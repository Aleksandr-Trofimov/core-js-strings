...
function formatTime(minutes, seconds) {
  ...
  const result = minutesStr
    .padStart(2, '0')
    .concat(':', secondsStr.padStart(2, '0'));
  return result;
}
...
function isPalindrome(str) {
  const strWithoutWhitespaces = str
    .toLocaleLowerCase()
    .replace(/[\s.,%!?]/g, '');
  const strWithoutWhitespacesReversed = strWithoutWhitespaces
    .split('')
    .reverse()
    .join('');
  if (strWithoutWhitespaces === strWithoutWhitespacesReversed) {
    return true;
  }
  return false;
}
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    if (arr[i].length > itemArrLength) {
      itemArrLength = arr[i].length;
      result = arr[i];
    }
  }
  return result;
...
function extractNameFromTemplate(value) {
  const newArr = value.replace(/[.,%!?]/g, '').split(' ');
  const result = newArr[1].concat(' ', newArr[2]);
  return result;
}
